...
herdr terminal session mirror <target> [--resume-from SEQ] [--cols N] [--rows N]
...
`terminal session mirror` opens a read-only replication stream for a pane,
terminal, or agent target. Unlike observe (which streams server-rendered
frames), mirror replicates the terminal's raw PTY output plus resize events with
durable sequence numbers, so a consumer can maintain its own terminal state
locally and scroll, search, and select without server round-trips. It prints a
newline-delimited JSON `mirror.snapshot` record (establishing `base_seq`, `cols`,
...
sends a fresh snapshot if that point is no longer retained. Multiple mirror
clients can watch the same terminal.
