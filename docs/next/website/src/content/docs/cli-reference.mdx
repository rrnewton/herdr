...
herdr terminal session mirror <target> [--json] [--resume-from SEQ] [--cols N] [--rows N]
...
`terminal session mirror` is a responsive local mirror of a pane, terminal, or
agent target. Unlike observe/control (which stream server-rendered frames),
mirror replicates the terminal's raw PTY output plus resize events with durable
sequence numbers, so the client keeps its own terminal emulator locally. By
default it is a full-screen interactive client: scrollback (mouse wheel, and
page keys off the alternate screen) is instant with no server round-trip, which
keeps scrolling fast on high-latency links; keystrokes are forwarded to the
remote terminal, and window resizes drive the remote size. If the connection
drops, the local view stays on screen and the client transparently reconnects,
resuming the stream from where it left off. Detach with `ctrl+b q`.

Pass `--json` for a headless consumer that prints newline-delimited JSON instead
of rendering: a `mirror.snapshot` record (establishing `base_seq`, `cols`,
`rows`), then `mirror.event` records (`output` with base64 raw bytes, or
`resize`), and a final `mirror.closed`. Pass `--resume-from SEQ` to continue a
previous stream after a disconnect: the server replays events after `SEQ`, or
sends a fresh snapshot if that point is no longer retained. Multiple read-only
mirror clients can watch the same terminal.
