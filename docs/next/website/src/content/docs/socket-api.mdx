...
Notification event subscriptions include `notification.shown`. The event fires
whenever Herdr raises an in-app notification/toast (e.g. an agent finishing or
needing attention, an installed update, or a `notification.show` request). It
carries the toast `kind` (`needs_attention`, `finished`, or `update_installed`),
the `title` and `context` display text, and the optional `workspace_id`/`pane_id`
the toast targets. Notifications are ephemeral; each client applies its own
display duration.
